...
\EndFor
\State sum all group roots and compare to the declared D4 or D5 enclosure
\State regenerate the formula constants, D5--D7 majorants, and exact tail
\State recompute every accepted-step contribution; require exact ledger equality
\State independently evaluate the previous step; require exact equality
\State require accepted total $\leq\eps$ and previous total $>\eps$
...
Deep mode inserts regeneration of all 31 baseline centers, the complete formula
logarithm, DSW defects, and the Pauli coefficient maps before the certificate
comparisons.  Importantly, both modes fail closed:
...
\path{candidate.d5_certificate} & object & Binds the exact D5 coefficient
sidecar, coverage, same-support groups, and site-norm enclosure \\
...
\text{step ratio}
  &=\frac{393}{95}=4.136842105263158\ldots,\\
\text{compiled resource ratio}
  &=\frac{30\cdot393+1}{30\cdot95+1}
    =\frac{11791}{2851}
...
\begin{equation}
 \frac{(11791/2851)}{2}=2.067870922483339\ldots.
\end{equation}
...
\item Confirm the exact integer map from 393/95 steps to all three resources.
\item Run the default unit suite, including mutation and local-Pauli tests.
\item Run the standard-library reference verifier to check D4/D5 partitions,
the full finite-step ledger, the adjacent boundary, and resource arithmetic.
\item If computational resources permit, run the deep verifier to regenerate
the formula-specific D4/D5 algebra and all 31 baseline centers.
