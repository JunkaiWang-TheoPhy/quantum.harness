...
smallest integer accepted by this certificate.
...
\code{11791/2851}.  Deep verification is invoked with \code{--deep}.  It
regenerates all D4 and D5 coefficients from the Suzuki formula and requires
exact interval equality with the two sidecars.  The observed deep run took
167.62 seconds, used at most 1,533,149,184 bytes of resident memory, and
returned \code{deep\_proof\_regenerated=true}.  The normal suite reports 100
passed tests and 11 explicitly deselected slow tests; focused minimality and
corruption-rejection checks report three passes.
